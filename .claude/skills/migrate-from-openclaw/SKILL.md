...
description: Migrate from OpenClaw to DelegateAgent. Detects existing OpenClaw installation, extracts identity, channel credentials, scheduled tasks, and other config, then guides interactive migration. Triggers on "migrate from openclaw", "openclaw migration", "import from openclaw".
---

# Migrate from OpenClaw

Guide the user through migrating their OpenClaw installation to DelegateAgent. This is a conversation, not a batch job. Read OpenClaw state, discuss it with the user, make judgment calls together about what to bring over and how.

**Principle:** Never silently copy data. Read it, explain it, discuss where it belongs in DelegateAgent's architecture, show proposed changes before applying. Credentials must be masked when displayed (first 4 + `...` + last 4 characters). Make judgment calls about what's core vs. reference material.
...
- Channels: list each, note which DelegateAgent supports (whatsapp, telegram, slack, discord) and which it doesn't
...
- **Container isolation:** DelegateAgent runs each agent in an isolated Linux container (Docker or Apple Container). OpenClaw runs everything in one process. This means stronger isolation but also means each group is its own sandbox.
- **Group-based memory:** In OpenClaw, all groups under one agent share the same SOUL.md, MEMORY.md, and IDENTITY.md. In DelegateAgent, each group has its own filesystem and CLAUDE.md. Shared state goes in `groups/global/CLAUDE.md` (mounted read-only into all non-main containers).
- **Channel skills:** In OpenClaw, channels are configured in `openclaw.json`. In DelegateAgent, channels are installed as code via skills (`/add-telegram`, `/add-whatsapp`, etc.) and configured through `.env` variables.
- **Simpler config:** DelegateAgent has no config file — behavior is in the code and `CLAUDE.md` files. Credentials live in `.env` or the OneCLI vault.
...
AskUserQuestion: "In OpenClaw, your groups shared the same personality and memory. In DelegateAgent, each group is a fully separate agent. How would you like to handle this?"
...
IDENTITY_NAME from discovery gives the OpenClaw name. Ask the user: "Your OpenClaw assistant was named `<IDENTITY_NAME>`. Want to keep this name in DelegateAgent?" If they want a different name, ask what it should be. If IDENTITY_NAME was empty, ask them to choose a name (default: "Andy").
...
The discovery script provides detected groups in the GROUPS field (format: `channel:id(name)=>delegate-agent_jid`). These are extracted from OpenClaw's session store and channel config.

For each group the user wants to bring over, pre-register it:

```bash
npx tsx setup/index.ts --step register -- --jid "<delegate-agent_jid>" --name "<group_name>" --folder "<channel>_<slug>" --trigger "@<confirmed_name>" --channel <channel> --assistant-name "<confirmed_name>"
```
...
Register groups from all channels — including channels DelegateAgent doesn't yet support (signal, matrix, etc.). The registration stores the JID and metadata in the database, ready for when that channel is added later. Groups won't receive messages until their channel code is installed, but the registration, group folder, and CLAUDE.md will be ready.

## Phase 2: Settings from Config

Before identity/memory, extract settings from `openclaw.json` that map directly to DelegateAgent setup. Read the config file with the Read tool (`<STATE_DIR>/openclaw.json` or `clawdbot.json`).

### Timezone

Check `agents.defaults.userTimezone` in the config. If present and it's a valid IANA timezone (e.g. `America/New_York`, `Asia/Jerusalem`), write it to `.env` as `TZ=<timezone>`. DelegateAgent's setup step 2a reads `TZ` from `.env` (`src/config.ts:84-97`) and will skip the autodetection prompt.
...
If found, offer to save to `.env`. This pre-fills the DelegateAgent setup credential step (step 4) so the user doesn't need to re-enter it. Use the same masking approach — show first 4 + last 4 characters, write the full value directly.
...
DelegateAgent uses `~/.config/delegate-agent/sender-allowlist.json` with this format:
```json
{
...
If OpenClaw had allowlists configured, show the user what was set and offer to create the DelegateAgent equivalent. Map:
...
For each detected plugin, present the name to the user and discuss whether to set it up in DelegateAgent. Read the OpenClaw config section to understand what it is, then:

1. **If DelegateAgent has a matching skill** — check the available DelegateAgent skills list for an equivalent (e.g. `/add-voice-transcription` for whisper). If found, save the API key to `.env` and invoke that skill.
...
- TOOLS.md: DelegateAgent agents have their own tool discovery; this doesn't transfer
- HEARTBEAT.md: DelegateAgent uses scheduled tasks instead
- BOOTSTRAP.md: DelegateAgent uses CLAUDE.md and container skills instead
- AGENTS.md: Already covered in the Phase 1 groups discussion

## Phase 4: Channel Credentials

For each channel found in the discovery results, handle it based on DelegateAgent support:

### Supported channels (whatsapp, telegram, slack, discord)

Run the credential extraction script with `--write-env .env` so it writes credentials directly to DelegateAgent's `.env` file. The script never emits raw credential values to stdout — only masked versions.
...
The script writes the credential directly to `.env` using the correct DelegateAgent variable name (e.g. `TELEGRAM_BOT_TOKEN`). Check the status block for `WRITTEN_TO` and `WRITTEN_COUNT` to confirm.
...
Explain briefly: "DelegateAgent doesn't have a `<channel>` integration yet, but channels are added over time via skills. Any groups from this channel were already registered in Phase 1 — they'll activate when the channel is added."
...
If jobs exist, read `${CLAUDE_SKILL_DIR}/MIGRATE_CRONS.md` for the full OpenClaw cron format, DelegateAgent table schema, field mapping, and SQL insert template. Follow those instructions for each job.
...
MCP servers in DelegateAgent are registered in the agent-runner source code. Before editing, grep for `mcpServers` in `container/agent-runner/src/` to find the current location — it's expected to be in `index.ts` in the `query()` options, but may have moved. For each OpenClaw MCP server the user wants to bring over:
...
- These don't map directly but DelegateAgent can be customized to support them
...
- **Exec approvals / command allowlist:** DelegateAgent uses container isolation instead — the agent runs with `--dangerously-skip-permissions` inside a sandboxed container
- **Human delay:** Not applicable in DelegateAgent's container model
- **Compaction:** Handled by Claude Code SDK automatically
- **TTS:** Not built into DelegateAgent
- **Model configuration:** DelegateAgent uses whatever Anthropic model the credential provides access to
...
- Sender allowlists → `~/.config/delegate-agent/sender-allowlist.json`
...
Remind: "Run `/setup` next to complete your DelegateAgent installation. Channel credentials are already prepared in `.env`. When setup asks which channels to enable, select the ones we configured."
...
**Multi-agent complexity:** If the user had many agents with different configs, focus on the primary/default agent first. Additional agents can be set up as separate DelegateAgent groups later.
