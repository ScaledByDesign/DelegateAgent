...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.delegate-agent.plist`) in the `EnvironmentVariables` dict if using launchd.
...
>
> Check logs: `tail -f logs/delegate-agent.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/delegate-agent.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.delegate-agent.plist && launchctl load ~/Library/LaunchAgents/com.delegate-agent.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart delegate-agent` (Linux)
