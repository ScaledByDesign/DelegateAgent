 ...
 *
 * Ported from Archon `packages/workflows/src/schemas/dag-node.ts`. Differences:
 * - Import from `zod` (not `@hono/zod-openapi`); the `.openapi('DagNode')` call
 *   is dropped — DA doesn't generate OpenAPI from these.
 * - `z.record(z.array(z.string()))` is replaced with the explicit two-arg
 *   `z.record(z.string(), z.array(z.string()))` form for `sandbox.ignoreViolations`
 *   and the `agents` map — plain zod 3.23+ requires both key and value schemas.
 * - Provider allowlist refine on `dagNodeBaseSchema.provider` (per plan Phase 1
 *   + R3): v1 accepts `claude` and `nanoclaw` only; any other value rejects at
 *   load with the explicit message tying authors to the Bifrost VK runbook.
 *
 * Design: a flat "raw" schema validates all fields (with mutual exclusivity
 * enforced via superRefine), then a transform produces one of seven concrete
 * variant types as the DagNode union. Per-variant schemas are exported for
 ...
 * so a flat schema with superRefine is cleaner than a z.union() with implicit
 * discriminants.
 */
import { z } from 'zod';
...
import { workflowNodeHooksSchema } from './hooks.js';
...
// ---------------------------------------------------------------------------
// Provider allowlist (v1 ships claude only — see plan Phase 1 + R3)
// ---------------------------------------------------------------------------

/** Allowlist for `provider:` strings in workflows.
 *  v1 accepts NanoClaw-routed Claude only; future providers add through the
 *  Bifrost VK gateway in Phase 2's `provider-bridge.ts`. */
export const ALLOWED_PROVIDERS = ['claude', 'nanoclaw'] as const;
...
// Kebab-case: no leading/trailing/double hyphens.
...
export const dagNodeBaseSchema = z.object({
  ...
  /**
   * Provider identity. v1 hard-allowlists `claude` and `nanoclaw` (both route
   * through the existing NanoClaw container path). Any other provider value
   * fails at load with a clear pointer to the Bifrost VK runbook — see plan
   * R3 and `bifrost_no_vk_lockdown_2026_05_11.md` memory.
   */
  provider: z
    .string()
    .trim()
...
/** A single node in a DAG workflow. The seven mode fields are mutually exclusive. */
export type DagNode =
  | CommandNode
  | PromptNode
  | BashNode
 ...
 *
 * Note: provider identity is validated on `dagNodeBaseSchema.provider` via
 * `.refine()` (allowlist `claude`/`nanoclaw` in v1). Model strings are
...
export const dagNodeSchema = dagNodeBaseSchema
  ...
  .superRefine((data, ctx) => {
    ...
    const hasBash =
      typeof data.bash === 'string' && data.bash.trim().length > 0;
  ...
  .transform((data): DagNode => {
    ...
    if (data.script !== undefined && data.script.trim().length > 0) {
      ...
      if (!data.runtime)
        throw new Error(
          'unreachable: runtime must be defined for script nodes',
        );
      return {
        ...base,
        ...shared,
...
// ---------------------------------------------------------------------------
// Type guards (preserved from upstream)
// ---------------------------------------------------------------------------
...
export function isTriggerRule(value: unknown): value is TriggerRule {
  return (
    typeof value === 'string' &&
    (TRIGGER_RULES as readonly string[]).includes(value)
  );
}
