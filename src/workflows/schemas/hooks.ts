 ...
 *
 * Ported from Archon `packages/workflows/src/schemas/hooks.ts`. Hook events
 * use the same names as the Claude Agent SDK's HookEvent type so a workflow
 * authored against Archon's docs ports unchanged.
 *
 * Note: v1 executor IGNORES the `hooks:` field per plan section 9 (Out of
 * scope). Schemas are present so future executor work doesn't need a schema
 * migration; the field passes through validation but has no runtime effect.
 */
import { z } from 'zod';
...
export const WORKFLOW_HOOK_EVENTS: readonly WorkflowHookEvent[] =
  workflowHookEventSchema.options;
